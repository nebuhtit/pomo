...
def split_file(whole, split_count=None, parts=None, save=True, destination=None):
	...
	if save:
		if destination is not None:
			name_of_file = os.path.basename(whole)
			destination_name = os.path.join(destination, name_of_file)
			for i, part_io in enumerate(parts_io):
				if parts is None:
					if filename_given:
						name = destination_name + '.{}.part'.format(i)
						print(name)
					else:
...
def combine_file(filename=None, parts=None, save=True, destination=None):
	...
	if save:
		if destination is not None:
			...
			with open(destination_name, 'wb') as whole:
